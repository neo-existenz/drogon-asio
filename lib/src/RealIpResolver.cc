...
struct XForwardedForParser : public NonCopyable
{
...
static InetAddress parseAddress(const std::string& addr)
{
    auto pos = addr.find(':');
    uint16_t port = 0;
    ...
    return InetAddress(addr.substr(0, pos), port);
}
...
void RealIpResolver::initAndStart(const Json::Value& config)
{
    ...
    drogon::app().registerPreHandlingAdvice([this](const HttpRequestPtr& req) {
        ...
        const InetAddress& peerAddr = req->getPeerAddr();
        ...
        {
            InetAddress addr = parseAddress(ipHeader);
...
const InetAddress& RealIpResolver::GetRealAddr(
    ...
    const HttpRequestPtr& req) const
{
    ...
    return attributesPtr->get<InetAddress>(attributeKey_);
}

bool RealIpResolver::matchCidr(const InetAddress& addr) const
{
