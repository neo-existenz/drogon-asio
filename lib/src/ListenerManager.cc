...
std::vector<drogon::InetAddress> ListenerManager::getListeners() const
{
    std::vector<drogon::InetAddress> listeners;
    ...
    if (!listeners_.empty())
    {
        ...
        for (auto const &listener : listeners_)
        {
            ...
            if (listener.useSSL_ && utils::supportsTls())
            {
                ...
                auto cmds = sslConfCmds;
                auto policy =
                    TLSPolicy::defaultServerPolicy(cert, key);
                policy->setConfCmds(cmds).setUseOldTLS(listener.useOldTLS_);
                serverPtr->enableSSL(std::move(policy));
            }
