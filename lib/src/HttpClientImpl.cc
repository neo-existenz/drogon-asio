...
void HttpClientImpl::createTcpClient()
{
    ...
    tcpClientPtr_->setSSLErrorCallback([weakPtr](SSLError err) {
        ...
        else if (err == SSLError::kSSLProtocolError)
...
static bool isValidIpAddr(const InetAddress &addr)
{
                                       ...
                                       drogon::HttpReqCallback &&callback)
{
    ...
    if (!tcpClientPtr_)
    {
        ...
        {
            resolverPtr_ =
                Resolver::newResolver(loop_, kDefaultDNSTimeout);
        }
        auto thisPtr = shared_from_this();
        resolverPtr_->resolve(
            domain_, [thisPtr](const InetAddress &addr) {
