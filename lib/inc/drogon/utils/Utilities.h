...
{
    return splitString(str, separator, acceptEmptyString);
}
...
/// Encode the string to base64 format with no padding.
inline std::string base64EncodeUnpadded(std::string_view data, bool url_safe = false)
{
 ...
 */
DROGON_EXPORT Date getHttpDate(const std::string &httpFullDateString);
...
{
    return utils::fromNativePath(strPath);
}
