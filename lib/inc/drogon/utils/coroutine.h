...
template <typename T = void>
struct CallbackAwaiter : public NonCopyable
{
    bool await_ready() noexcept
    {
        return false;
...
template <>
struct CallbackAwaiter<void> : public NonCopyable
{
    bool await_ready() noexcept
    {
        return false;
...
{
    TimerAwaiter(EventLoop *loop,
                 const std::chrono::duration<double> &delay)
        : loop_(loop), delay_(delay.count())
    {
    }
    TimerAwaiter(EventLoop *loop, double delay)
...
{
    LoopAwaiter(EventLoop *workLoop,
                std::function<void()> &&taskFunc,
                EventLoop *resumeLoop = nullptr)
        : workLoop_(workLoop),
  ...
  private:
    EventLoop *workLoop_{nullptr};
    EventLoop *resumeLoop_{nullptr};
    std::function<void()> taskFunc_;
};

struct [[nodiscard]] SwitchThreadAwaiter : CallbackAwaiter<void>
{
    explicit SwitchThreadAwaiter(EventLoop *loop) : loop_(loop)
    {
    }
    void await_suspend(std::coroutine_handle<> handle)
    {
...
{
    EndAwaiter(EventLoop *loop) : loop_(loop)
    {
        assert(loop);
    }
    void await_suspend(std::coroutine_handle<> handle)
...
inline internal::TimerAwaiter sleepCoro(EventLoop *loop,
...
inline internal::EndAwaiter untilQuit(EventLoop *loop)
{
    assert(loop);
    return {loop};
}
...
{
    EventLoopAwaiter(std::function<T()> &&task, EventLoop *loop)
 ...
 */
template <typename T>
inline internal::EventLoopAwaiter<T> queueInLoopCoro(EventLoop *loop,
                                                     std::function<T()> task)
{
