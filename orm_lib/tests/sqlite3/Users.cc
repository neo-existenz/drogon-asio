...
const std::vector<typename Users::MetaData> Users::metaData_ = {
    ...
    {"create_time", "Date", "datetime", 0, 0, 0, 0}};
    ...
    if (indexOffset < 0)
    {
        ...
        if (!r["create_time"].isNull())
        {
            ...
            if (p)
            {
                ...
                createTime_ =
                    std::make_shared<Date>(t * 1000000 + decimalNum);
            }
        }
    }
    else
...
const Date &Users::getValueOfCreateTime() const noexcept
{
    const static Date defaultValue = Date();
    if (createTime_)
        return *createTime_;
    return defaultValue;
}
const std::shared_ptr<Date> &Users::getCreateTime() const noexcept
{
    return createTime_;
}
void Users::setCreateTime(const Date &pCreateTime) noexcept
{
    createTime_ = std::make_shared<Date>(pCreateTime);
    dirtyFlag_[9] = true;
}
