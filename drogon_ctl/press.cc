...
void press::doTesting()
{
    ...
    statistics_.startDate_ = drogon::Date::now();
...
{
    loopPool_ = std::make_unique<drogon::EventLoopThreadPool>(numOfThreads_);
    loopPool_->start();
    ...
    client->sendRequest(
        ...
        [this, client, request](ReqResult r, const HttpResponsePtr &resp) {
            ...
            if (r == ReqResult::Ok)
            {
                ...
                auto delay = drogon::Date::now().microSecondsSinceEpoch() -
